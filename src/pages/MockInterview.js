...
export default function MockInterview() {
  ...
  useEffect(() => {
    if (isRecording) {
      timerRef.current = setInterval(() => {
        setTimer((t) => {
          if (t + 1 >= q.timeLimit) { stopRecording(); return t + 1; }
          return t + 1;
        });
      }, 1000);
    } else {
  ...
  const goTo = useCallback((next) => {
    ...
    setTimeout(() => { setScreen(next); setTransitioning(false); }, 280);
  }, [transitioning]);
  ...
  const screenClass = (i) => screen === i ? 'active' : screen > i ? 'above' : 'below';

  return (
    <div className={`interview-wrap ${transitioning ? 'transitioning' : ''}`} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      <div className="app-frame">
        <div className="nav-dots">
          {[0,1,2].map((i) => <div key={i} className={`nav-dot ${screen === i ? 'active' : ''}`} />)}
        </div>
        ...
        <div className={`screen screen-question ${screenClass(0)}`}>
          <div className="screen-inner">
            <div className="flashcard">
              ...
              <div className="question-number">Question {q.id} of {QUESTIONS.length}</div>
            ...
            <div className="swipe-hint"><div className="swipe-arrow" />Swipe up to answer</div>
          </div>
        </div>
        ...
        <div className={`screen screen-recording ${screenClass(1)}`}>
          <div className="screen-inner">
            ...
            <div className="question-pill">{q.question.length > 72 ? q.question.slice(0, 72) + '…' : q.question}</div>
            <div className="waveform-container"><Waveform isActive={isRecording} /></div>
            ...
            {isRecording ? (
              <button className="stop-button" onClick={stopRecording}><div className="stop-icon" />Stop &amp; Analyze</button>
            ) : (
            ...
            <div className="swipe-hint"><div className="swipe-arrow" />Swipe up to finish</div>
          </div>
        </div>
          ...
          <div className="screen-inner">
            <div className="feedback-header">
              ...
              <div className="overall-score">
                {isAnalyzing ? <span className="analyzing-label">Analyzing…</span> : (
            ...
            <div className="score-grid">
              {Object.entries(scores).map(([label, score], i) => <ScoreRing key={label} score={score} label={label} delay={i * 0.1} />)}
            </div>
            <div className="ai-insight">
              <div className="ai-insight-header"><span className="ai-badge">AI</span><span className="ai-insight-label">Key Insight</span></div>
